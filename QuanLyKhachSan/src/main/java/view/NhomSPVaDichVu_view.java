...
//	private int _sttNhanVienEditing;
	...
	public void doClickOnTable() {
		...
		for (NhomSPVaDichVuModel nhomSPVaDichVuModel : _listNhomSPVaDichVuModels) {
			if(nhomSPVaDichVuModel.getMaNhomSP() == maNhom) {
				txt_maNhom.setText(nhomSPVaDichVuModel.getMaNhomSP()+"");
				txt_tenNhom.setText(nhomSPVaDichVuModel.getTenNhomSP());
				if(nhomSPVaDichVuModel.getTrangThai().equals("Hoạt Động")) {
					switchButton_Nhap.setSelected(true);
				}else {
					switchButton_Nhap.setSelected(false);
				}
				cbx_nhomHangHoa.setSelectedItem(nhomSPVaDichVuModel.getNhomHangHoa());
				txt_ghiChu.setText(nhomSPVaDichVuModel.getGhiChu());
